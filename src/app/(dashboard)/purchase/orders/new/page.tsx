...
interface Product { id: string; name: string; sku: string; purchase_price: number }
...
export default function NewPurchaseOrderPage() {
  ...
  const onProductChange = (i: number, productId: string) => {
    ...
    setItems(prev => prev.map((it, idx) =>
      idx === i ? { ...it, product_id: productId, description: prod ? `${prod.name} (${prod.sku})` : it.description, unit_price: prod ? String(prod.purchase_price) : it.unit_price } : it
    ))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(d.error || 'Failed to create purchase order')
      router.push(`/purchase/orders/${d.data.id}`)
    } catch (err: any) {
