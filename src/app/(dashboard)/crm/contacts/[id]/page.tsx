...
export default function ContactDetailPage() {
  ...
  return (
    <div>
      <PageHeader title={contact.name} description="Contact details" />
      ...
      <div className="max-w-2xl rounded-lg border bg-card p-6">
        <dl className="space-y-3 text-sm">
          ...
          <div className="flex justify-between"><dt className="text-muted-foreground">WhatsApp</dt><dd>{contact.whatsapp || '-'}</dd></div>
          <div className="flex justify-between"><dt className="text-muted-foreground">Company</dt><dd>{companyId ? <Link href={`/crm/companies/${companyId}`} className="hover:text-primary">{companyName}</Link> : companyName}</dd></div>
          <div className="flex justify-between"><dt className="text-muted-foreground">Position</dt><dd>{contact.position || '-'}</dd></div>
          <div className="flex justify-between"><dt className="text-muted-foreground">Created</dt><dd>{contact.created_at ? new Date(contact.created_at).toLocaleDateString() : '-'}</dd></div>
        </dl>
        {contact.notes && (
