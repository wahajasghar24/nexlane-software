...
export async function GET(request: Request) {
  try {
    ...
    await authorize(context, Permissions.EMPLOYEES_LIST)

    const url = new URL(request.url)
    const query = Object.fromEntries(url.searchParams.entries())

    const data = await employeeService.list(context.companyId, query)
...
export async function POST(request: Request) {
  try {
    const context = await authenticate(request)
    await authorize(context, Permissions.EMPLOYEES_CREATE)

    const body = createEmployeeSchema.parse(await request.json())
    const data = await employeeService.create(context.companyId, body, context.userId)
