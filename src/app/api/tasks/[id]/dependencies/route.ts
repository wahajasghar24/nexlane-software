...
import { addTaskDependencySchema } from '@/features/tasks/schemas'
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const data = await taskService.listDependencies(context.companyId, id)
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params
    const body = addTaskDependencySchema.parse(await request.json())
    const data = await taskService.addDependency(context.companyId, id, body, context.userId)
