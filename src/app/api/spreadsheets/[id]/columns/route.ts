...
import { createSheetColumnSchema } from '@/features/spreadsheets/schemas/sheet-table.schema'
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = createSheetColumnSchema.parse({ sheet_table_id: id, ...(await request.json()) })
    const data = await sheetTableService.addColumn(context.companyId, body)
