...
export async function GET(request: Request) {
  try {
    ...
    const data = await sheetTableService.list(context.companyId, { search, page, limit })
...
export async function POST(request: Request) {
  try {
    const context = await authenticate(request)
    await authorize(context, Permissions.SPREADSHEETS_CREATE)

    const body = createSheetTableSchema.parse(await request.json())
    const data = await sheetTableService.create(context.companyId, body, context.userId)
