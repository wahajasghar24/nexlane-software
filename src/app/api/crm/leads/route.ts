...
import { createLeadSchema, leadQuerySchema } from '@/features/crm/schemas'
import { ZodError } from 'zod'

export async function GET(request: Request) {
  try {
    const context = await authenticate(request)
    await authorize(context, Permissions.LEADS_LIST)

    const url = new URL(request.url)
    const query = leadQuerySchema.parse(Object.fromEntries(url.searchParams.entries()))

    const data = await leadService.list(context.companyId, query)
...
export async function POST(request: Request) {
  try {
    const context = await authenticate(request)
    await authorize(context, Permissions.LEADS_CREATE)

    const body = createLeadSchema.parse(await request.json())
    const data = await leadService.create(context.companyId, body, context.userId)
