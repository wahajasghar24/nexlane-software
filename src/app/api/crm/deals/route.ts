...
import { createDealSchema, dealQuerySchema } from '@/features/crm/schemas'
import { ZodError } from 'zod'

export async function GET(request: Request) {
  try {
    const context = await authenticate(request)
    await authorize(context, Permissions.DEALS_LIST)

    const url = new URL(request.url)
    const query = dealQuerySchema.parse(Object.fromEntries(url.searchParams.entries()))

    const data = await dealService.list(context.companyId, query)
...
export async function POST(request: Request) {
  try {
    const context = await authenticate(request)
    await authorize(context, Permissions.DEALS_CREATE)

    const body = createDealSchema.parse(await request.json())
    const data = await dealService.create(context.companyId, body, context.userId)
