...
} from '@/features/sales/schemas/sales-order.schema'
import type {
...
export const salesOrderService = {
  async generateOrderNumber(companyId: string): Promise<string> {
    ...
    const now = new Date()
    const prefix = `SO-${now.getFullYear()}${padNum(now.getMonth() + 1, 2)}-`
  ...
  async list(companyId: string, query: SalesOrderQuery): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    let dbQuery = supabase
      ...
      .select(`
        *,
        ...
        invoice:invoice_id(id, invoice_number, status, total)
      `, { count: 'exact' })
      .eq('company_id', companyId)
      .is('deleted_at', null)
  ...
  async create(companyId: string, input: CreateSalesOrderInput, actorId: string) {
  ...
  async update(companyId: string, orderId: string, input: UpdateSalesOrderInput, actorId: string) {
    const parsed = updateSalesOrderSchema.parse(input)
    const supabase = await createClient()
    ...
    if (existing.status !== 'draft') {
      throw new AppError('CONFLICT', 'Only draft quotations can be edited', 409)
    }
    ...
    if (parsed.items) {
      const { error: delErr } = await supabase.from('sales_order_items').delete().eq('order_id', orderId)
  ...
  /**
   * Odoo-style cross-module confirm:
   * draft (quotation) -> confirmed; each item writes a -qty stock_movement
   * (stock deducted), and a draft invoice is auto-created and linked.
   */
  async confirm(companyId: string, orderId: string, actorId: string) {
    const supabase = await createClient()
    ...
    if (!order) throw new AppError('CONFLICT', 'Only draft quotations can be confirmed', 409)
    ...
    // 1. Stock out for every product line
    ...
    // 2. Auto-create draft invoice (cross-module link)
    const invoiceNumber = await accountingService.generateInvoiceNumber(companyId)
    ...
    // 3. Mark confirmed + link invoice
    const { data: updated, error: upErr } = await supabase
      .from('sales_orders')
      .update({ status: 'confirmed', invoice_id: invoice.id, updated_by: actorId })
      .eq('id', orderId)
      .eq('company_id', companyId)
      .select()
      .single()
